...
public class BinaryTreeView extends View {
    ...
    private ArrayList<Integer> insertSequence = null;
    ...
    public void initialize() {
        clear();
        while(insertPosition < insertSequence.size()) {
            nextStep(false);
        }
    ...
    public void nextStep(boolean invalidate) {
        tree.insert(insertSequence.get(insertPosition));
        insertPosition++;
        if(invalidate) {
            tree.positionNodes(this.getWidth());
            invalidate();
    ...
    public void clear() {
        ...
        insertSequence = generateRandomSequence(TREE_SIZE);
        insertPosition = 0;
    }
                ...
                case MotionEvent.ACTION_DOWN:
                    ...
                    if (hitValue != -1) {
                        ...
                        if (hitValue != targetValue) {
                            ...
                            int indexOfValue = searchSequence.indexOf(hitValue);
                            searchSequence.remove(indexOfValue);
                        }
                        searchPosition++;
                        updateMessage();
                        return true;
